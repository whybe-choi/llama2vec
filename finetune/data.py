...
class TrainDatasetForEmbedding(Dataset):
    def __init__(self, args: DataArguments, tokenizer: PreTrainedTokenizer):
        ...
        self.passage_max_len = (
            self.args.passage_max_len
            - len(self.prefix_ids)
            - len(self.suffix_passage_ids)
        )
    ...
    def __getitem__(self, item) -> Tuple[BatchEncoding, List[BatchEncoding]]:
        ...
        # in paper, use the deductive embedding for query.
        # input format for query : <s> "{query}" predict the following passage within eight words: <s9><s10><s11><s12><s13><s14><s15><s16>
        ...
        # num of positive example + negative examples = train_group_size
        # if the number of negative examples is less than the group size, repeat the negative examples
        if len(self.dataset[item]["neg"]) < self.args.train_group_size - 1:
            num = math.ceil(
                (self.args.train_group_size - 1)
                / len(list(set(self.dataset[item]["neg"])))
            )
            negs = random.sample(
        ...
        for passage in passages:
            ...
            # in paper, use the inductive embedding for passage.
            # input format for passage : <s> "{passage}" summarize the above passage within eight words: <s1><s2><s3><s4><s5><s6><s7><s8>
    ...
    def __call__(self, features, return_tensors="pt"):
        ...
        if self.sub_batch_size is None or self.sub_batch_size <= 0:
            ...
            for i in range(0, len(queries), batch_size):
                start = i
                # if i + batch_size > len(queries), end = len(queries)
