...
class ModelArguments:
    ...
    lora_alpha: float = field(
        default=16, metadata={"help": "The alpha parameter of lora."}
    )
    lora_dropout: float = field(
        default=0.1, metadata={"help": "The dropout rate of lora modules."}
    )
...
class RetrieverTrainingArguments(TrainingArguments):
    ...
    temperature: Optional[float] = field(
        default=0.02, metadata={"help": "temperature for the softmax"}
    )
    ...
    normalized: bool = field(
        default=True, metadata={"help": "if passed, normalize the embeddings"}
    )
    sub_batch_size: int = field(
        default=None, metadata={"help": "sub batch size for the retriever"}
    )
    cache_chunk_size: int = field(
        default=-1,
        metadata={"help": "chunk size for the retriever cache per each training step."},
    )
